...
- (BOOL)forwardConnectionToRemoteFromSocket:(libssh2_socket_t)forwardsock listensock:(libssh2_socket_t)listensock shost:(const char *)shost sport:(unsigned int)sport host:(NSString *)remoteHost port:(NSInteger)remotePort {
    ...
    // Open a channel
    LIBSSH2_CHANNEL *channel = libssh2_channel_direct_tcpip_ex(self.session.rawSession, [remoteHost UTF8String], (int)remotePort, shost, sport);
    
    if (channel == NULL) {
        NMSSHLogError(@"Unable to open a channel");
        goto shutdown;
    }
    ...
    for(;;) {
        ...
        if(-1 == rc) {
            NMSSHLogError(@"failed to select().");
            goto shutdown;
        }
        if(rc && FD_ISSET(forwardsock, &fds)) {
            len = recv(forwardsock, buf, sizeof(buf), 0);
            if(len < 0) {
                NMSSHLogError(@"failed to recv().");
                goto shutdown;
            }
            else if(len == 0) {
                NMSSHLogError(@"The client at %s:%d disconnected.", shost,
                        sport);
                goto shutdown;
            }
            wr = 0;
            while(wr < len) {
                ...
                if(nwritten < 0) {
                    NMSSHLogError(@"libssh2_channel_write: %d",
                            (int)nwritten);
                    goto shutdown;
                }
                wr += nwritten;
        ...
        for(;;) {
            ...
            if(LIBSSH2_ERROR_EAGAIN == len)
                ...
                NMSSHLogError(@"libssh2_channel_read: %d", (int)len);
                goto shutdown;
            }
            wr = 0;
            while(wr < len) {
                ...
                if(nsent <= 0) {
                    NMSSHLogError(@"failed to send().\n");
                    goto shutdown;
                }
                wr += nsent;
            }
            if(libssh2_channel_eof(channel)) {
                NMSSHLogVerbose(@"The server at %@:%ld disconnected.", remoteHost, (long)remotePort);
                goto shutdown;
            }
        }
    }
...
+ (struct NMSSHSocket)createSocket {
    return (struct NMSSHSocket){socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)};
}

- (BOOL)bindLocalPortToRemoteHostWithSocket:(struct NMSSHSocket)sock localListenIP:(NSString *)localListenIP localPort:(NSInteger)localPort host:(NSString *)remoteHost port:(NSInteger)remotePort in:(dispatch_queue_t)queue error:(NSError *__autoreleasing *)error {
    NMSSHLogInfo(@"Opening direct TCP/IP connection to %@:%ld", remoteHost, (long)remotePort);
    
    dispatch_queue_t listenQueue = dispatch_queue_create("socket_listen", NULL);
    int sockopt;
    socklen_t sinlen;
    struct sockaddr_in sin;
    NSString *errorMessage;
    
    libssh2_socket_t listensock = sock.sock;
    
    sin.sin_family = AF_INET;
    sin.sin_port = htons((unsigned short) localPort);
    sin.sin_addr.s_addr = inet_addr([localListenIP UTF8String]);
    if(INADDR_NONE == sin.sin_addr.s_addr) {
        errorMessage = [NSString stringWithFormat:@"failed in inet_addr(). (%i)", errno];
        NMSSHLogError(@"%@", errorMessage);
        goto finish;
    }
    sockopt = 1;
    ...
    if(-1 == bind(listensock, (struct sockaddr *)&sin, sinlen)) {
        errorMessage = [NSString stringWithFormat:@"failed to bind(). (%i)", errno];
        NMSSHLogError(@"%@", errorMessage);
        goto finish;
    }
    if(-1 == listen(listensock, 2)) {
        errorMessage = [NSString stringWithFormat:@"failed to listen(). (%i)", errno];
        NMSSHLogError(@"%@", errorMessage);
        goto finish;
    }
    ...
    if (errorMessage) {
        *error = [NSError errorWithDomain:@"NMSSH"
                                    code:NMSSHChannelSocketError
                                    userInfo:@{
                                       NSLocalizedDescriptionKey : errorMessage,
                                       NSUnderlyingErrorKey: [NSError errorWithDomain:NSPOSIXErrorDomain code:errno userInfo: NULL]
                                       }
          ];
        return NO;
    }
    ...
    dispatch_async(listenQueue, ^{
        ...
        for(;;){
    listen:
            NMSSHLogVerbose(@"Waiting for TCP connection on %s:%d...\n",
                            inet_ntoa(sin.sin_addr), ntohs(sin.sin_port));
            
            forwardsock = accept(listensock, (struct sockaddr *)&sin, &sinlen);
            if(forwardsock == LIBSSH2_INVALID_SOCKET) {
                NMSSHLogError(@"failed to accept forward socket.");
                goto shutdown;
            }
            ...
            NMSSHLogVerbose(@"Forwarding connection from %s:%li here to remote %@:%li",
                            shost, (long)sport, remoteHost, (long)remotePort);
            
            dispatch_group_t group = dispatch_group_create();
            dispatch_group_enter(group);
            
            dispatch_sync(queue, ^{
                [self forwardConnectionToRemoteFromSocket:forwardsock listensock:listensock shost:shost sport:sport host:remoteHost port:remotePort];
    ...
    shutdown:
        ...
        // FIXME: sockets are not clos
        close(listensock);
        if (self.socketDelegate && [self.socketDelegate respondsToSelector:@selector(socketDidClose:)]) {
            [self.socketDelegate socketDidClose:sock];
        }
    });
    return YES;
}
